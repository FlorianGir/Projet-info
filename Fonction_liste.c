...
L_LEX ajout_queue_lex(L_LEX l_lex, LEXEME lex, MOT l_mot, int ligne) {
...
void Virgule(L_LEX lex, MOT* mot, int ligne) {		//! Passage de mot par adresse car modifie dans la fonction (pareil pour les 4 fonctions suivantes)
...
void New_Line(L_LEX lex, MOT* mot, int* ligne) {
	...
	*mot = ajout_queue_mot(*mot, '\n');
	...
	lex = ajout_queue_lex(lex, NL, *mot, *ligne);
...
void Par_o(L_LEX lex, MOT *mot, int ligne) {
	...
	*mot = ajout_queue_mot(*mot, '(');
	...
	lex = ajout_queue_lex(lex, PAR_O, *mot, ligne);
...
void Par_f(L_LEX lex, MOT* mot, int ligne) {
	...
	*mot = ajout_queue_mot(*mot, ')');
	...
	lex = ajout_queue_lex(lex, PAR_F, *mot, ligne);
...
void Deux_pts(L_LEX lex, MOT *mot, int ligne){
	...
	*mot = ajout_queue_mot(*mot, ':');
	...
	lex = ajout_queue_lex(lex, DEUX_PTS, *mot, ligne);
...
L_LEX memory_etiquette(L_LEX l_lex) {
	L_LEX l_etiquette = NULL;
	L_LEX q = l_lex;

	while (q != NULL) {
		if (q->lex == ETIQUETTE) {
			l_etiquette = ajout_queue_lex(l_etiquette, ETIQUETTE, q->mot, q->ligne);
		}
		q = q->suiv;
	}
	return l_etiquette;
